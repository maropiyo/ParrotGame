...
namespace TedLab
{
    public class CaptureScreenShotWindow : EditorWindow
    {
        ...
        IEnumerator<float> MultipleCaptureScreen()
        {
            m_started = true;
        ...
        {
            public static EditorCoroutine Start( IEnumerator<float> _routine )
	        {
		        var coroutine = new EditorCoroutine(_routine);
		        coroutine.Start();
		        return coroutine;
	        }
            readonly IEnumerator<float> routine;
            float _waitTimer = 0f;
            double _prevTime = 0;
		    EditorCoroutine( IEnumerator<float> _routine ){
			    routine = _routine;
		    }
